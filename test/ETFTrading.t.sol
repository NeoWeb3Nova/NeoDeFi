...
contract ETFTradingTest is Test {
    ...
    uint256 public constant USDC_PER_SHARE = 177.6 * 10 ** 6; // 177.60 USDC (~$177.60, 20%)
    ...
    function mintTokensToUser(address _userAddress, uint256 _etfShareMultiplier) internal {
        ...
        console.log("NBTC: ", FormatUtils.formatTokenAmount((NBTC_PER_SHARE * _etfShareMultiplier), NBTC_DECIMALS));
        console.log("NETH: ", FormatUtils.formatTokenAmount((NETH_PER_SHARE * _etfShareMultiplier), NETH_DECIMALS));
        console.log("LINK: ", FormatUtils.formatTokenAmount((LINK_PER_SHARE * _etfShareMultiplier), LINK_DECIMALS));
        console.log("USDC: ", FormatUtils.formatTokenAmount((USDC_PER_SHARE * _etfShareMultiplier), USDC_DECIMALS));
    }

    function approveTokensForETFTrading(address _userAddress, uint256 _etfShareMultiplier) internal {
        vm.prank(_userAddress);
        ERC20(NBTC_TOKEN).forceApprove(address(etfTrading), NBTC_PER_SHARE * _etfShareMultiplier);
        vm.prank(_userAddress);
        ERC20(NETH_TOKEN).forceApprove(address(etfTrading), NETH_PER_SHARE * _etfShareMultiplier);
        vm.prank(_userAddress);
        ERC20(LINK_TOKEN).forceApprove(address(etfTrading), LINK_PER_SHARE * _etfShareMultiplier);
        vm.prank(_userAddress);
        ERC20(USDC_TOKEN).forceApprove(address(etfTrading), USDC_PER_SHARE * _etfShareMultiplier);

        console.log("User approved ETFTrading to spend their tokens");
        console.log(
            "Approved NBTC: ", FormatUtils.formatTokenAmount((NBTC_PER_SHARE * _etfShareMultiplier), NBTC_DECIMALS)
        );
        console.log(
            "Approved NETH: ", FormatUtils.formatTokenAmount((NETH_PER_SHARE * _etfShareMultiplier), NETH_DECIMALS)
        );
        console.log(
            "Approved LINK: ", FormatUtils.formatTokenAmount((LINK_PER_SHARE * _etfShareMultiplier), LINK_DECIMALS)
        );
        console.log(
            "Approved USDC: ", FormatUtils.formatTokenAmount((USDC_PER_SHARE * _etfShareMultiplier), USDC_DECIMALS)
        );
    }
    ...
    function setUp() public {
        ...
        etfTrading =
            new ETFTrading(ETF_NAME, ETF_SYMBOL, tokenAddresses, tokenAmounts, MIN_MINT_AMOUNT, UNISWAP_V3_SWAP_ROUTER);
    ...
    function testInvestTokenAmounts() public view {
        ...
        uint256[] memory amounts = etfTrading.getInvestTokenAmount(testMintAmount);
        ...
        console.log("NBTC: ", FormatUtils.formatTokenAmount(amounts[0], NBTC_DECIMALS));
        console.log("NETH: ", FormatUtils.formatTokenAmount(amounts[1], NETH_DECIMALS));
        console.log("LINK: ", FormatUtils.formatTokenAmount(amounts[2], LINK_DECIMALS));
        console.log("USDC: ", FormatUtils.formatTokenAmount(amounts[3], USDC_DECIMALS));
    }

    function testSetFeeOnlyOwner() public {
        vm.prank(userAddress);
        vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, userAddress));
    ...
    function testInvestWithToken() public {
        ...
        (uint256 investAmount, bytes[] memory swapPath) =
            etfQuoter.quoteInvestWithToken(address(etfTrading), NBTC_TOKEN, mintAmount);
        ...
        vm.prank(userAddress);
        etfTrading.investWithToken(NBTC_TOKEN, userAddress, mintAmount, maxInvestAmount, swapPath);
        vm.stopPrank();

        uint256 userEtfBalance = etfTrading.balanceOf(userAddress);
        uint256 expectedEtfBalance = mintAmount - (mintAmount * 1000) / 10000000; // Deducting 0.01% invest fee
        assertEq(userEtfBalance, expectedEtfBalance, "User ETF balance should match the minted amount");

        console.log("User invested %d and received %d ETF shares:", userEtfBalance, expectedEtfBalance);

        uint256 feeCollectorEtfBalance = etfTrading.balanceOf(feeCollectorAddress);
        uint256 expectedFeeCollectorBalance = (mintAmount * 1000) / 10000000; // 0.01% invest fee
        assertEq(
            feeCollectorEtfBalance, expectedFeeCollectorBalance, "Fee collector ETF balance should match the invest fee"
        );
    }
    ...
    function testRedeemToToken() public {
        ...
        console.log(
            ...
            FormatUtils.formatTokenAmount(userEtfBalanceBeforeRedeem, ETF_DECIMALS)
        );

        (uint256 redeemAmountOut, bytes[] memory redeemSwapPath) =
            etfQuoter.quoteRedeemToToken(address(etfTrading), NBTC_TOKEN, userEtfBalanceBeforeRedeem);
        ...
        uint256 userNBTCBalanceBeforeRedeem = ERC20(NBTC_TOKEN).balanceOf(userAddress);
        console.log(
            "User NBTC balance before redeeming:",
            FormatUtils.formatTokenAmount(userNBTCBalanceBeforeRedeem, NBTC_DECIMALS)
        );

        uint256 minRedeemAmount = (redeemAmountOut * 95) / 100; // 5% slippage tolerance
        vm.startPrank(userAddress);
        etfTrading.redeemToToken(NBTC_TOKEN, userAddress, userEtfBalanceBeforeRedeem, minRedeemAmount, redeemSwapPath);
        vm.stopPrank();

        uint256 userEtfBalanceAfterRedeem = etfTrading.balanceOf(userAddress);
        console.log(
            "User ETF balance after redeeming:", FormatUtils.formatTokenAmount(userEtfBalanceAfterRedeem, ETF_DECIMALS)
        );
        uint256 userNBTCBalanceAfterRedeem = ERC20(NBTC_TOKEN).balanceOf(userAddress);
        console.log(
            "User NBTC balance after redeeming:",
            FormatUtils.formatTokenAmount(userNBTCBalanceAfterRedeem, NBTC_DECIMALS)
        );

        assertEq(userEtfBalanceAfterRedeem, 0, "User ETF balance should be zero after redeeming all shares");
        ...
        uint256 receivedNBTCAmount = userNBTCBalanceAfterRedeem - userNBTCBalanceBeforeRedeem;
        console.log(
