...
contract ETFQuoter is IETFQuoter {
        ...
        returns (uint256 srcTokenAmount, bytes[] memory path)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == srcToken) {
                ...
                path[i] = bytes.concat(bytes20(srcToken), bytes3(fees[0]), bytes20(srcToken));
            } else {
                (bytes memory _path, uint256 _amountIn) = quoteExactOut(srcToken, tokens[i], tokenAmount[i]);
                path[i] = _path;
        ...
        returns (uint256 dstTokenAmount, bytes[] memory path)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == dstToken) {
                ...
                path[i] = bytes.concat(bytes20(dstToken), bytes3(fees[0]), bytes20(dstToken));
            } else {
                (bytes memory _path, uint256 _amountOut) = quoteExactIn(tokens[i], dstToken, tokenAmount[i]);
                path[i] = _path;
        ...
        returns (bytes memory path, uint256 amountOut)
    {
        ...
        for (uint256 i = 0; i < paths.length; i++) {
            try uniswapV3Quoter.quoteExactInput(paths[i], amountIn) returns (
                ...
                if (_amountOut > 0 && (amountOut == 0 || _amountOut > amountOut)) {
        ...
        returns (bytes memory path, uint256 amountIn)
    {
        ...
        for (uint256 i = 0; i < paths.length; i++) {
            try uniswapV3Quoter.quoteExactOutput(paths[i], amountOut) returns (
                uint256 _amountIn, uint160[] memory, uint32[] memory, uint256
            ) {
    ...
    function getAllPaths(address srcToken, address dstToken) public view returns (bytes[] memory paths) {
        paths = new bytes[](fees.length);
        for (uint256 i = 0; i < fees.length; i++) {
            paths[i] = bytes.concat(bytes20(srcToken), bytes3(fees[i]), bytes20(dstToken));
        }
    }
}
